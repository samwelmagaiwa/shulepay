...
class InstallmentResource extends JsonResource
{
    public function toArray($request): array
    {
        ...
        $nextDueDate = \App\Models\InstallmentPlan::where('invoice_id', $this->invoice_id)
            ->where('status', '!=', 'paid')
            ->orderBy('due_date')
            ->value('due_date');
        ...
        return [
            ...
            'next_due_date'           => $nextDueDate instanceof \Carbon\Carbon
                                            ? $nextDueDate->format('Y-m-d')
                                            : ($nextDueDate ? \Carbon\Carbon::parse($nextDueDate)->format('Y-m-d') : null),
            'status'                  => $this->status,
